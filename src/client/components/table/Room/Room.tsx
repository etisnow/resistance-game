...
const Room = observer(({controller} : IRoomProps) => {
	...
	// нулевое место. Сам разворот — общий с рукой (см. roomPlayerOrder): по нему
	// же она считает, из чьего кружка прилетает и в чей улетает карта обмена.
	const anchorId = isBurningSelf ? prevSeats.current[1] : currentPlayerId;
	const seatedList = roomPlayerOrder(
		playersList,
		anchorId ?? '',
		controller.isLayoutSequential && (currentPlayer.turnState !== ETurnState.dead || isBurningSelf),
	);
