 ...
 *
 * Это сжатие пола: по нему живут круг рассадки, тени под игроками и тень самого
 * стола. Столешница поднята над полом и потому видна чуть иначе — см.
 * tableTopSquash.
 *
 * Круг на полу задника нарисован ровно с этим сжатием, и стол ставится в его
 * середину (см. RoomBackdrop): камера у комнаты и у стола обязана быть одна.
 */
 ...
 *
 * Высота столешницы над полом, в долях большой полуоси круга рассадки. Из неё
 * следуют обе поправки: столешница поднята на экране (tableLift) и видна под
 * более пологим углом, чем пол (tableTopSquash).
 */
const tableStandShare = 0.055;

/**
 * Высота глаза над полом в тех же долях.
 *
 * Ею одной задаётся, насколько положе мы смотрим на столешницу, чем на пол: от
 * глаза до пола — вся эта высота, а до столешницы — она же минус высота стола,
 * и круг на ней сжимается ровно во столько же раз слабее. Пол при высоте глаза
 * вровень со столешницей выродился бы в линию — стол именно поэтому и виден
 * «площе» комнаты, в которой стоит.
 */
const cameraEyeShare = 0.38;

/**
 * Сжатие круга на высоте столешницы. Всё, что лежит НА столе — сама столешница,
 * колода, карты, паника, тень под колодой, — живёт по нему, а не по сжатию пола.
 */
export const tableTopSquash = tableSquash * (1 - tableStandShare / cameraEyeShare);
