...
export class Player {
	...
	isSpy: boolean = false;
	// Мерлин: сопротивленец, которому видно, кто шпион. Шпионы его не знают — в
	// этом вся игра за него (FR-14). Бывает только в партии с Мерлином.
	isMerlin: boolean = false;
	// Убийца: шпион, который в случае поражения шпионов стреляет в того, кого
	// считает Мерлином (FR-15). Свои его знают, сопротивление — нет.
	isAssassin: boolean = false;
	// Персиваль: сопротивленец, которому показывают Мерлина — но вместе с
	// Морганой и не говоря, кто из них кто (FR-16).
	isPercival: boolean = false;
	// Моргана: шпионка, которую Персиваль видит наравне с Мерлином. В этом вся её
	// роль — заставить его сомневаться.
	isMorgana: boolean = false;
